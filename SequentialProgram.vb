...
Public Class SequentialWorkflow
    ...
    Function PreviousWorkDay(ByVal curDate As DateTime) As String
        ...
        Return curDate.ToString("yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture)
    End Function
        ...
        While idx < dataList.Count
            ...
            If Not data Is Nothing Then
                ...
                Try
                    ...
                    Dim expenddt As Date = DateTime.Now

                    Dim articleDate As String = expenddt.Day.ToString("d2") + expenddt.Month.ToString("d2")


                    Dim Xml_Crossword As Xml_Transform = New Xml_Transform()
                    myxmlOut = Xml_Crossword.Process(data.LocalPath)
                    Dim crossWordName As String = data.Info.SrcNameNoExtension.Substring(data.Info.SrcNameNoExtension.Length - 8)
                    Dim crossWordDate As Date = DateTime.ParseExact(crossWordName, "yyyyMMdd", Globalization.CultureInfo.InvariantCulture)
                    ...
                    Dim imgExtract As Image_Extract = New Image_Extract()
                    imgExtract.Extract(data.LocalPath, imgPhysicalPath)
                    Dim crosswordImages = imgExtract.ExtractBase64(data.LocalPath)

                    'Upload images to GN
                    Dim imgsXml As XElement = <objects>
                                                  <img name=<%= crossWordName & "_grid" %> source="Back4">
                                                  ...
                                                  <img name=<%= imgSolutionDateStr & "_sol" %> source="Back4">
                                                      <folderRef><keyVal><%= imgFolderPath %></keyVal></folderRef>
                                                      <low mime="image/jpeg">
                                                          <data>
                                                              <%= crosswordImages(1) %>
                    ...
                    findImg.Pars.Add("imgName", crossWordName & "_sol")
                    ...
                    CreateArticle(crossWordXml, crossWordName + " - SOxWord", True, folderPath, articleImgIds, textFormat, typography, title)
